...
### 3. Project-Specific Steering Guide
- **DuaSaku Guide:** For any implementation, refactoring, or modification in this workspace, you MUST read, understand, and strictly follow the patterns and rules in [.kiro/steering/duasaku.md](file:///c:/Codingg/duasaku_app/.kiro/steering/duasaku.md). It takes precedence over general mobile rules if they conflict.
...
Setiap kali mengimplementasikan fitur baru, perubahan struktural, atau melakukan refactoring besar, Anda **HARUS** membuat folder baru di bawah `docs/specs/<feature-slug>/` yang berisi 3 berkas utama berikut sebelum mulai menulis kode implementasi:
...
- **Key Skills**: `clean-code`, `brainstorming`, `app-builder`, `frontend-design`, `mobile-design`, `design-audit`, `plan-writing`, `behavioral-modes`, `caveman`, `rtk`
...
1.  **Centralize**: Keep this folder at `D:\Coding\ag-kit`.
2.  **Symlink**: Reference the `.agent` folder from your project root.
    - **PowerShell (Admin)**: `New-Item -ItemType SymbolicLink -Path ".agent" -Target "D:\Coding\ag-kit\.agent"`
    - **CMD (Admin)**: `mklink /D .agent "D:\Coding\ag-kit\.agent"`

class LocalDateParser {
  ...
  static DateTime? parse(String text, {DateTime? referenceDate}) {
    ...
    final daysAgoMatch = RegExp(r'\b(\d+)\s*(?:hari|hr)\s*(?:lalu|kemar[ei]n|yang\s+lalu)?\b').firstMatch(cleanText);
    if (daysAgoMatch != null) {
      final days = int.tryParse(daysAgoMatch.group(1) ?? '');
      if (days != null) {
        return DateTime(ref.year, ref.month, ref.day - days, ref.hour, ref.minute);
      }
    }

    // Weeks ago (e.g. "3 minggu kemaren", "3 mgg")
    final weeksAgoMatch = RegExp(r'\b(\d+)\s*(?:minggu|mgg|mg)\s*(?:lalu|kemar[ei]n|yang\s+lalu)?\b').firstMatch(cleanText);
    if (weeksAgoMatch != null) {
      final weeks = int.tryParse(weeksAgoMatch.group(1) ?? '');
      if (weeks != null) {
        return DateTime(ref.year, ref.month, ref.day - (weeks * 7), ref.hour, ref.minute);
      }
    }

    // Singular weeks ago: "minggu lalu", "minggu kemaren"
    if (RegExp(r'\bminggu\s*(?:lalu|kemar[ei]n|yang\s+lalu)\b').hasMatch(cleanText)) {
      return DateTime(ref.year, ref.month, ref.day - 7, ref.hour, ref.minute);
    }

    // Months ago (e.g. "1 bulan lalu", "1 bln")
    final monthsAgoMatch = RegExp(r'\b(\d+)\s*(?:bulan|bln)\s*(?:lalu|kemar[ei]n|yang\s+lalu)?\b').firstMatch(cleanText);
    ...
    if (RegExp(r'\bbulan\s*(?:lalu|kemar[ei]n|yang\s+lalu)\b').hasMatch(cleanText)) {
    ...
    if (RegExp(r'\b(?:tadi|td)\s+malam\b').hasMatch(cleanText)) {
      ...
      return DateTime(ref.year, ref.month, ref.day - 1, 20, 0); // Yesterday 8 PM
    }
    ...
    for (final entry in weekdayMap.entries) {
      if (RegExp('\\b${entry.key}\\b').hasMatch(cleanText)) {
        ...
        return DateTime(ref.year, ref.month, ref.day - diff, ref.hour, ref.minute);
      }
    }

    // 5. Standard Numeric Dates (DD/MM/YYYY, DD-MM-YYYY, DD.MM.YYYY, DD/MM/YY)
    // Try DD/MM/YYYY first
    final numericDateMatch = RegExp(r'\b(\d{1,2})[/\-\.](\d{1,2})[/\-\.](\d{2,4})\b').firstMatch(cleanText);
    ...
    final numericDayMonthMatch = RegExp(r'\b(\d{1,2})[/\-\.](\d{1,2})\b').firstMatch(cleanText);
    ...
    if (namedMonthMatch != null) {
      ...
      if (day != null && monthStr != null) {
        ...
        if (month != null && day >= 1 && day <= 31) {
          final year = yearStr != null ? (int.tryParse(yearStr) ?? ref.year) : ref.year;
          return DateTime(year, month, day, 12, 0);
        }
      }
    }
  ...
  static DateTime _subtractMonths(DateTime dt, int months) {
    ...
    return DateTime(y, m, d, dt.hour, dt.minute, dt.second, dt.millisecond, dt.microsecond);
  }
  ...
  static int? _mapMonth(String monthStr) {
    ...
    if (clean.startsWith('agu') || clean.startsWith('ags') || clean.startsWith('agt') || clean.startsWith('aug')) return 8;

...
class NotificationParserService {
  final Ref _ref;
  static const EventChannel _channel = EventChannel('com.duasaku.app/bank_notifications');
  ...
  void _processNotification(String packageName, String title, String text) {
    debugPrint('[NotificationParserService] Received: $packageName | $title | $text');
    ...
    final timestampMinute = '${now.year}-${now.month}-${now.day} ${now.hour}:${now.minute}';
    ...
    if (_processedSignatures.contains(signature)) {
      debugPrint('[NotificationParserService] Duplicate detected, ignoring. ($signature)');
      return;
    }
  ...
  Map<String, dynamic>? _parseText(String packageName, String title, String text) {
    ...
    if (packageName.contains('com.bca') || lowerTitle.contains('bca')) {
      bank = 'BCA';
    } else if (packageName.contains('mandiri') || lowerTitle.contains('mandiri') || lowerTitle.contains('livin')) {
      bank = 'Mandiri';
    } else if (packageName.contains('bri') || lowerTitle.contains('brimo')) {
      bank = 'BRI';
    } else if (packageName.contains('jago')) {
      bank = 'Jago';
    } else if (packageName.contains('gojek') || packageName.contains('gopay') || lowerTitle.contains('gopay')) {
      bank = 'GoPay';
    ...
    if (combined.contains('dana masuk') || 
      ...
      if (!combined.contains('berhasil') && !combined.contains('sukses') && !combined.contains('masuk')) {
          return null;
      }
    }

    // Extract amount
    final amountPattern = RegExp(r'rp\s?(\d{1,3}(?:\.\d{3})*(?:,\d{2})?)', caseSensitive: false);
  ...
  Future<void> _insertTransaction(double amount, String type, String bankName, String title, String text) async {
    final user = _ref.read(userProvider);
    if (user == null) {
      debugPrint('[NotificationParserService] User not logged in, cannot insert.');
      return;
    }
    ...
    try {
      await _ref.read(transactionNotifierProvider.notifier).createTransaction(
        amount: amount,
        category: 'Notification ($bankName)',
        type: type,
        notes: notes,
      );
      debugPrint('[NotificationParserService] Recorded transaction of Rp $amount as $type');
    } catch (e) {

...
/// Bottom sheet with step-by-step flow for creating a recurring transaction.
///
/// Flow: amount → type → category → wallet → frequency → dates → preview → confirm
class CreateRecurringBottomSheet extends ConsumerStatefulWidget {
  const CreateRecurringBottomSheet({super.key});

  @override
  ConsumerState<CreateRecurringBottomSheet> createState() =>
      _CreateRecurringBottomSheetState();
  ...
  static Future<void> show(BuildContext context) {
    return showModalBottomSheet(
      ...
      builder: (_) => const CreateRecurringBottomSheet(),
    );
  }
}
...
class _CreateRecurringBottomSheetState
    extends ConsumerState<CreateRecurringBottomSheet>
    with SingleTickerProviderStateMixin {
  final _pageController = PageController();
  _Step _currentStep = _Step.amount;
  ...
  ReminderTiming _reminderTiming = ReminderTiming.sameDay;
  ...
  void _nextStep() {
    ...
    if (nextIndex < _Step.values.length) {
      _goToStep(_Step.values[nextIndex]);
    }
  }
  ...
  void _previousStep() {
    ...
    if (prevIndex >= 0) {
      _goToStep(_Step.values[prevIndex]);
    } else {
  ...
  bool _validateDates() {
    ...
    if (startDay.isBefore(today)) {
      setState(() => _dateError = 'recurring.error.start_date_past'.tr());
      return false;
    }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('recurring.error.creation_failed'.tr())),
      );
    }
  }
  ...
  Widget _buildStepIndicator(ColorScheme colorScheme) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 8),
      child: Row(
        children: List.generate(_Step.values.length, (index) {
        ...
        children: [
            ...
            child: ListWheelScrollView.useDelegate(
              ...
              onSelectedItemChanged: (index) {
                setState(() {
                  ...
                  // Reset interval if it exceeds new max
          ...
          Row(
            children: [
              ...
              IconButton(
                onPressed:
                    _customInterval < _selectedFrequency.maxInterval
                        ? () => setState(() => _customInterval++)
                        : null,

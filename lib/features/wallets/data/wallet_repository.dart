...
class WalletRepository implements WalletRepositoryInterface {
  ...
  Future<Result<List<WalletModel>, AppError>> getWallets(String userId) async {
    try {
      final rows = await (_db.select(_db.wallets)..where((tbl) => tbl.userId.equals(userId))).get();
      return Success(rows.map((w) => WalletModel(
        id: w.id,
        userId: w.userId,
        name: w.name,
        type: w.type,
  ...
  Stream<List<WalletModel>> watchWallets(String userId) {
    return (_db.select(_db.wallets)..where((tbl) => tbl.userId.equals(userId))).watch().map((rows) {

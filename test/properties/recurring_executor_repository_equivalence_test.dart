    ...
    Glados2(
      ...
      (initialBalance, amount) async {
        ...
        try {
          ...
          await _seedWallet(dbExecutor,
              walletId: walletId, initialBalance: initialBalance);
          await _seedCategory(dbExecutor, categoryId: categoryId);

          await _seedWallet(dbRepo,
              walletId: walletId, initialBalance: initialBalance);
          ...
          await _seedRecurringTransaction(
            ...
            amount: amount,
            type: 'income',
            nextExecutionDate: DateTime.now().subtract(const Duration(hours: 1)),
          );
          ...
          final walletExecutor = await (dbExecutor.select(dbExecutor.wallets)
                ..where((w) => w.id.equals(walletId)))
              .getSingle();
          final walletRepo = await (dbRepo.select(dbRepo.wallets)
                ..where((w) => w.id.equals(walletId)))
              .getSingle();
    ...
    Glados2(
      ...
      (initialSourceBalance, amount) async {
        ...
        try {
          ...
          // --- Set up identical databases ---
          await _seedWallet(dbExecutor,
              walletId: sourceWalletId, initialBalance: initialSourceBalance);
          await _seedWallet(dbExecutor,
              walletId: destWalletId, initialBalance: initialDestBalance);
          await _seedCategory(dbExecutor, categoryId: categoryId);
          ...
          await dbExecutor.transaction(() async {
            await dbExecutor.into(dbExecutor.transactions).insert(
            ...
            final sourceWallet =
                await (dbExecutor.select(dbExecutor.wallets)
                      ..where((w) => w.id.equals(sourceWalletId)))
                    .getSingle();
            await (dbExecutor.update(dbExecutor.wallets)
                  ..where((w) => w.id.equals(sourceWalletId)))
          ...
          final sourceRepo = await (dbRepo.select(dbRepo.wallets)
                ..where((w) => w.id.equals(sourceWalletId)))
              .getSingle();
          final destRepo = await (dbRepo.select(dbRepo.wallets)
                ..where((w) => w.id.equals(destWalletId)))
              .getSingle();
          ...
          final destDeltaExecutor =
              destExecutor.balance - initialDestBalance;

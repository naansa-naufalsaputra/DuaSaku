...
void main() {
  ...
  group('SQLite Database Stress & Reliability Tests', () {
    ...
    test(
      'Stress Test: 1000 Concurrent Transactions & Balance Consistency',
      () async {
        const numWallets = 5;
        ...
        for (final result in results) {
          expect(
            result,
            isA<Success>(),
            reason: 'Concurrent transaction insertion failed',
          );
        }
        ...
        for (final walletId in walletIds) {
          ...
          expect(
            ...
            reason:
                'Database balance mismatch for wallet $walletId after 200 operations',
          );
        }
    ...
    test('Reliability Test: Transaction Rollback on Error', () async {
      ...
      try {
        await db.transaction(() async {
          ...
          await (db.update(db.wallets)..where((w) => w.id.equals(walletId)))
              .write(const WalletsCompanion(balance: Value(300.0)));
      ...
      final dbWallet = await (db.select(
        db.wallets,
      )..where((w) => w.id.equals(walletId))).getSingle();
      expect(
        dbWallet.balance,
        equals(500.0),
        reason: 'Wallet balance was NOT rolled back after transaction crash',
      );
      ...
      expect(
        txRows.length,
        equals(0),
        reason: 'Transaction table was NOT rolled back after transaction crash',
      );
    });
  });
}

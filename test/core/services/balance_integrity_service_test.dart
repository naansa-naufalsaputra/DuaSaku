...
void main() {
  ...
  group('BalanceIntegrityService Unit Tests', () {
    ...
    test('checkAllWallets detects discrepancy between stored and computed balance', () async {
    ...
    test('checkAllWallets ignores minor discrepancies below threshold', () async {
    ...
    test('repairWallet fixes balance discrepancy', () async {
      ...
      final wallet = await (db.select(db.wallets)..where((w) => w.id.equals('wallet-4'))).getSingle();
    ...
    test('repairAllDiscrepancies fixes multiple wallets', () async {
      ...
      for (final wallet in wallets) {
        if (wallet.id == 'wallet-5') expect(wallet.balance, closeTo(500.0, 0.01));
        if (wallet.id == 'wallet-6') expect(wallet.balance, closeTo(1000.0, 0.01));
      }
    });

    test('_computeBalance correctly calculates balance with all transaction types', () async {
      // Create wallet
      await db.into(db.wallets).insert(
        WalletsCompanion.insert(
          id: 'wallet-7',

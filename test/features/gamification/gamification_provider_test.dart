  ...
  group('GamificationNotifier Unit Tests', () {
    ...
    ProviderContainer createContainer({
      ...
      return ProviderContainer(
        overrides: [
          budgetNotifierProvider.overrideWith(() => MockBudgetNotifier(budgets)),
          transactionNotifierProvider.overrideWith(() => MockTransactionNotifier(transactions)),
          walletProvider.overrideWith(() => MockWalletNotifier(wallets)),
        ],
      );
    }

    test('initializes state with defaults when SharedPreferences is empty', () async {
      ...
      expect(state.healthScore, equals(55)); // 40 points default S_budget + 15 points default S_saving when empty
    });

    test('restores streak and badges from SharedPreferences correctly', () async {
      SharedPreferences.setMockInitialValues({
        'user_streak_days': 5,
    ...
    test('logDailyActivity increments streak when logged in on consecutive days', () async {
      final yesterdayStr = DateTime.now().subtract(const Duration(days: 1)).toIso8601String();
      SharedPreferences.setMockInitialValues({
        'user_streak_days': 3,
    ...
    test('logDailyActivity resets streak when last active was over 1 day ago', () async {
      final daysAgoStr = DateTime.now().subtract(const Duration(days: 3)).toIso8601String();
      SharedPreferences.setMockInitialValues({
        'user_streak_days': 5,
    ...
    test('calculates healthScore correctly based on budget overspend and saving ratio', () async {
      ...
      final budgets = [
        BudgetProgress(budget: mockBudget, spent: 500.0),
      ];
      ...
      final wallets = [
        WalletModel(id: 'w1', userId: 'u1', name: 'BCA', type: 'Bank', balance: 10000.0, createdAt: DateTime.now()),
        WalletModel(id: 'w2', userId: 'u1', name: 'Cash', type: 'Cash', balance: 500.0, createdAt: DateTime.now()),
      ];
    ...
    test('awards badges correctly based on streak and transaction count thresholds', () async {
      SharedPreferences.setMockInitialValues({

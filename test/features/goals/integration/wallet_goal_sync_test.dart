...
Future<void> _updateWalletBalance(
  ...
  await (db.update(db.wallets)..where((t) => t.id.equals(walletId))).write(
...
void main() {
  ...
  group('Wallet balance change propagates to linked goal', () {
    test(
      ...
      () async {
        ...
        final wallet = await (db.select(
          db.wallets,
        )..where((t) => t.id.equals('wallet-1'))).getSingle();
    ...
    test(
      'wallet balance sync caps goal currentAmount at targetAmount',
      () async {
        ...
        expect(fetchedGoal.currentAmount, equals(500.0));
        expect(
          fetchedGoal.currentAmount,
          lessThanOrEqualTo(fetchedGoal.targetAmount),
        );
      },
    );

    test(
      'wallet balance decrease updates goal currentAmount accordingly',
      () async {
  ...
  group('Wallet deletion switches goal to manual mode', () {
    ...
    test('wallet deletion retains the last known currentAmount', () async {
      ...
      await (db.delete(db.wallets)..where((t) => t.id.equals('wallet-1'))).go();
    ...
    test(
      'wallet deletion does not affect goals linked to other wallets',
      () async {
  ...
  group('Wallet already linked to active goal prevents re-linking', () {
    test(
      'isWalletLinked returns true when wallet is linked to an active goal',
      () async {
    ...
    test(
      'isWalletLinked returns false when wallet is not linked to any goal',
      () async {

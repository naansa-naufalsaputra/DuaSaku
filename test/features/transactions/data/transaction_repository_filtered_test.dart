...
void main() {
  ...
  group('fetchTransactionsFiltered - Search Query', () {
    test('filters transactions by searchQuery in notes', () async {
      ...
      final stream = repository.fetchTransactionsFiltered(testUserId, filters, 100);
      final transactions = await stream.first;
      
      expect(transactions.length, greaterThan(0));
      expect(
        transactions.every((t) => 
          t.notes.toLowerCase().contains('makan')
        ),
        isTrue,
      );
    });
  ...
  group('fetchTransactionsFiltered - Date Range', () {
    ...
    test('filters by date range (both startDate and endDate)', () async {
      ...
      expect(
        transactions.every((t) => 
          (t.createdAt.isAfter(startDate) || t.createdAt.isAtSameMomentAs(startDate)) &&
          (t.createdAt.isBefore(endDate) || t.createdAt.isAtSameMomentAs(endDate))
        ),
        isTrue,
      );
    });
  ...
  group('fetchTransactionsFiltered - Amount Range', () {
    test('filters by minAmount', () async {
      ...
      expect(
        transactions.every((t) => t.amount >= 50000),
        isTrue,
      );
    });
    ...
    test('filters by maxAmount', () async {
      ...
      expect(
        transactions.every((t) => t.amount <= 100000),
        isTrue,
      );
    });
...
Future<void> _seedTestData(AppDatabase db, String userId) async {
  ...
  final testTransactions = [
    // Income
    {'notes': 'Salary June', 'amount': 5000000.0, 'type': 'income', 'date': DateTime(2026, 6, 1)},
    {'notes': 'Freelance payment', 'amount': 1500000.0, 'type': 'income', 'date': DateTime(2026, 6, 5)},
    
    // Expenses - Food related
    {'notes': 'Makan siang di resto', 'amount': 75000.0, 'type': 'expense', 'date': DateTime(2026, 6, 10)},
    {'notes': 'Makan malam keluarga', 'amount': 150000.0, 'type': 'expense', 'date': DateTime(2026, 6, 12)},
    {'notes': 'Grocery shopping', 'amount': 350000.0, 'type': 'expense', 'date': DateTime(2026, 6, 14)},
    
    // Expenses - Other
    {'notes': 'Transport', 'amount': 50000.0, 'type': 'expense', 'date': DateTime(2026, 6, 11)},
    {'notes': 'Electricity bill', 'amount': 200000.0, 'type': 'expense', 'date': DateTime(2026, 6, 13)},
    {'notes': 'Internet subscription', 'amount': 350000.0, 'type': 'expense', 'date': DateTime(2026, 6, 15)},
    
    // Varied amounts for range testing
    {'notes': 'Small expense', 'amount': 15000.0, 'type': 'expense', 'date': DateTime(2026, 6, 16)},
    {'notes': 'Large expense', 'amount': 500000.0, 'type': 'expense', 'date': DateTime(2026, 6, 17)},
  ];
